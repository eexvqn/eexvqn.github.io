...
plot_ly(historical_emissions, x = ~year) %>%
  add_trace(y = ~(agriculture * 100 / total), type = 'bar', name = 'Agriculture', marker = list(color = '#E1AAD1')) %>%
  add_trace(y = ~(land_use_change_and_forestry * 100 / total), type = 'bar', name = 'Land Use Change & Forestry', marker = list(color = '#DBE5A9')) %>%
